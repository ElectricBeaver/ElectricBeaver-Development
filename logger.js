...
client.once('message', msg => {
    ...
    fs.readdir(`./logs/${guild}/`, (err, files) => {
        if (err) console.log(err);
        if (!fs.existsSync(`./logs/${guild}/${guild}${'-' + date}.yml`)) {
            fs.writeFile(`./logs/${guild}/${guild}${'-' + date}.yml`, 'init', function (err) {
                if (err) throw err;
            })
        }
    ...
    try {
        log4js.configure({
            ...
            appenders: {
                ...
                file: { type: 'file', filename: `./logs/${guild}/${guild}${'-' + date}.yml` },
        ...
        msg.attachments.forEach(attachments => {
            ...
            if (!msg.content.startsWith(config.setup.prefix) && !msg.author.bot) {
                if (url) {
                    ...
                    if(msg.guild.id === '565225856604045322'){
